...
router.post('/create_payment_url', function (req, res, next) {
  ...
  //   res.status(302).set('Location', vnpUrl);
  res.status(200).json(vnpUrl);
});

router.get('/vnpay_return', function (req, res, next) {
  console.log('dã chạy vnpay_return');
...
router.post('/add-course-order', async (req, res) => {
  try {
    ...
    // Tạo một CourseOrder mới
    const newCourseOrder = new CourseOrder({
      courseName,
      amount,
      bankCode,
      receiveUser,
    ...
    // Lưu vào database
    const savedOrder = await newCourseOrder.save();
    ...
    console.error('Error adding CourseOrder:', error);
    return res.status(500).json({ message: 'Lỗi server, không thể thêm CourseOrder!' });
  }
});

router.get('/vnpay_ipn', function (req, res, next) {
  console.log('dã chạy vnpay_ipn');
